...
function QuickPurchase() {
  ...
  const handleProceedClick = () => {
    ...
    const updateQuantity = (productId, newQuantity, prevQuantity) => {
      // Handle empty quantity field - just clear the field but keep product in cart
      if (newQuantity === "") {
        ...
        // Also update cartItems (localStorage)
        setCartItems(prev => ({
          ...prev,
          [productId]: {
            ...prev[productId],
        ...
        // Update productCategories to keep in sync
      ...
      if (isNaN(quantityNum)) {
        setCartProducts(prevProducts =>
          prevProducts.map(product =>
            product.id === productId
              ? { ...product, quantity: prevQuantity, subtotal: product.price * prevQuantity }
              : product
          )
        );
        return;
      ...
      setProductCategories(prevCategories => 
        prevCategories.map(category => ({
          ...category,
          products: category.products.map(p => 
            p.id === productId ? {...p, quantity: quantityNum} : p
          )
        }))
      );
    };
...
const addressText = `${formData.address}, ${formData.city}, ${formData.district || formData.city}, ${formData.state}, ${formData.pinCode}`;
...
if (formData.email) {
  ...
  try {
   ...
   const handleQuantityChange = (productId, value) => {
    ...
    if (value === '') {
        // Update product categories to clear quantity
        setProductCategories((prev) => {
        ...
        // Remove from cartItems (localStorage)
        setCartItems((prev) => {
            const newCart = { ...prev };
        ...
        // Remove from cartProducts if exists
        setCartProducts(prevProducts =>
    ...
    // If invalid number, keep the field value but don't update cart
            ...
            if (existingProduct) {
                ...
                return [
                    ...prevProducts,
                    {
                        ...
                        price: productDetails.discountPrice,
                        originalPrice: productDetails.prodPrice,
                        subtotal: productDetails.discountPrice * newQuantity,
                        image: productDetails.prodImage,
                        discountPercent: getDiscountPercent(
...
>
    ...
    <div style={{
        ...
        if (productCount === 0) {
            alert("No items selected. Please add at least one product to proceed.");
            return;
        }
...
<div style={{ ...styles.card, flex: isMobile ? 1 : 3 }}>
  ...
  // In the remove button onClick handler:
onClick={() => {
  ...
  // Also update productCategories to reset quantity
  setProductCategories(prevCategories => 
    prevCategories.map(category => ({
      ...category,
      products: category.products.map(p => 
...
<input
  type="number"
  min="1" // Changed from type="number" to allow empty value
  value={product.quantity || ""}
  onChange={(e) => {
  ...
  onBlur={(e) => {
    // When field loses focus, ensure minimum quantity is 1 if not empty
...
<button
  ...
  onClick={() => {
    // Always: close both cart and form
    setShowViewCart(false);
    setShowDetailsForm(false);

    // Optional: Scroll back to top of products
    const productTable = document.querySelector('.product-table-anchor');
    if (productTable) {
      const headerHeight = document.querySelector('header')?.offsetHeight || 0;
      window.scrollTo({
