...
import pro_bg from "../assets/PO.jpg";
...
const handleConfirmOrder = async () => {
  ...
  try {
    ...
    if (!termsCheckbox.checked) {
      ...
      return; // Stop execution if terms aren't agreed to
    }
    ...
    if (whatsappNumber && !/^\d{10}$/.test(whatsappNumber)) {
      throw new Error("WhatsApp Number must be exactly 10 digits if provided.");
    }
    ...
    if (response.data && response.data.statusCode === 200) {
      ...
      throw new Error(response.data?.message || "Order submitted, but response was unclear. Please contact support.");
    }
  } catch (error) {
...
// Helper function to reset form and cart
  ...
  // };
    ...
    const updateQuantity = (productId, newQuantity, prevQuantity) => {
      ...
      if (newQuantity === "" || isNaN(quantityNum)) {
        // Update cartProducts (UI)
        setCartProducts(prevProducts =>
          prevProducts.map(product =>
            product.id === productId
              ? { ...product, quantity: "", subtotal: 0 }
              : product
          )
        );
    
        // Update cartItems and quantities to reflect empty state
        setCartItems(prev => {
          const newCart = { ...prev };
          if (newCart[productId]) {
            newCart[productId] = { ...newCart[productId], quantity: 1 }; // or keep as 1?
          }
          return newCart;
        });
        ...
        setQuantities(prev => {
          const newQuantities = { ...prev };
          delete newQuantities[productId]; // Clear quantity in catalog
          return newQuantities;
        });
      ...
      // Case 2: Quantity is 0 → trigger removal
      if (quantityNum === 0) {
      ...
      // ✅ Also update cartItems and quantities for consistency
      setCartItems(prev => {
 ...
 <li key={product.id} style={{ ...styles.productItem, position: 'relative' }}>
    ...
    <button 
        ...
        onClick={() => {
          ...
          // ✅ Clear from quantities so product card resets
          setQuantities(prev => {
            const newQuantities = { ...prev };
          ...
          <div style={{
            ...
            <div style={{ 
              ...
              fontSize: '0.65rem' 
               ...
               ₹{(product.originalPrice * product.quantity).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
            <div style={{ 
              fontSize: '0.8rem',
              fontWeight: 'bold'
            }}>
              <div>₹{product.subtotal.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
            </div>
          </div>
        )}
      </div>
